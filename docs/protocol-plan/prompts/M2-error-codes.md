# Prompt — M2: Foutcodes & errorenvelope

Dekt fase **M2** uit [`../README.md`](../README.md#fasering). Deze module heeft
geen inhoudelijke afhankelijkheid van **M1** — sterker, M1's
`assertPayloadSize`-fixturetest wacht juist op "een bestaande generieke
afwijzing uit de Input-categorie van `error-codes` (M2)" (`../README.md`, fase
M1): welke van de 23 codes daar precies van toepassing is, wordt hier
vastgesteld, niet in M1 aangenomen. M1 en M2 zijn dus onafhankelijk van elkaar
...
[`../README.md`](../README.md), fase M2, aanhoudt: "Eén typed enum met alle 23
...
`../README.md`, fase M2, over `buildErrorPayload`: "garandeert dat `meta` nooit
...
`../README.md`, fase M2, over de contracttest: "elke code in de enum komt
...
code? Blokkeert een deel van M2."

## Te bouwen functies

Voorgestelde locatie: `server/protocol/` (of de bij protocol-plan's eigen
M0-checkpoint bevestigde map — `../README.md`, fase M0). Dit voorstel gaat uit
...
M0-checkpoint vóórdat je buiten `docs/` iets aanmaakt. Schrijf platte
...
Verdeel dit over minstens drie bestanden (enum, `buildErrorPayload`,
contracttest) plus hun tests, binnen de autonomie-limiet van 5 bestanden/400
regels per actie (`CLAUDE.md`); splits verder in meerdere commits als dat niet
past.
 ...
 * volledige antwoordpayload (`../README.md`, fase M2).
 ...
 * envelope-module (M1) toegevoegd, niet hier.
 *
...
- Koppeling van `buildErrorPayload` aan de volledige server→client
  ...
  `envelope`-module (M1); deze functie levert uitsluitend `payload.code` en
  `payload.meta`.
...
- REST-foutresponses (HTTP-statuscodes, response-bodyvorm buiten de
  socket-`error`-eventvorm) — dat hoort bij `rest-games` (M3).
