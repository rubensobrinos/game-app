# Prompt — M7: Contracttest-suite tegen fake transport

Dekt fase **M7** uit [`../README.md`](../README.md#fasering), inclusief de
sub-batchindeling **M7a–M7e** exact zoals daar beschreven. Vereist M1 (afgerond,
`server/protocol/envelope.mjs` + `idempotency.mjs`) en idealiter M2–M6; waar een van
...
- [`../README.md`](../README.md), fase **M7** en **Open vragen §2**.
- `server/protocol/envelope.mjs`, `server/protocol/idempotency.mjs` (bestaande M1-code
...
`../README.md`, fase M7, letterlijk (de vijf sub-batches):
...
> - **M7c** — REST-scenario (de 5 endpoints, inclusief de `auth-shape`-header) tegen
>   M3.
> - **M7d** — client-/server-eventscenario (inclusief de snapshot-invariant en de
>   `round:progress`-throttle) tegen M4/M5.
...
**Wat dit betekent voor M7e:** de restart-scenario-test toetst wat ondubbelzinnig
...
aanmaakt, zoals ook bij M0/A0 in de zusterplannen gebeurt voor nieuwe locaties.
...
draait hier al; dit levert alleen de bouwstenen voor M7b–M7e.
 ...
 * `restart()` simuleert een serverherstart (M7e): alle in-memory
 ...
 * hergebruik in M7b–M7e — geen nieuwe idempotentielogica, alleen een fake
 * bewaarplaats.
 *
...
#### M7b — envelope/idempotentie-scenario (tegen M1/M2)

```js
/**
 * Draait create → join → `round:answer` met dubbele `actionId` →
 * idempotente ack, tegen de fake-transportlaag (M7a) en de échte M1-code
 * (`parseClientEnvelope`, `buildAck`, `resolveDuplicateAction` uit
 * `server/protocol/envelope.mjs`/`idempotency.mjs`). Als
 * `server/protocol/error-codes.mjs` (M2) nog niet bestaat op het moment van
 * uitvoeren: bouw eerst M2 (zie `../README.md`) voor de `ALREADY_ANSWERED`-
...
#### M7c — REST-scenario (tegen M3)

```js
/**
 * Draait de 5 REST-endpointscenario's (create, join, state, leave, time)
 * tegen de fake-Fastify-stub (M7a) en M3's schema's/validators
 * (`server/protocol/rest-games.mjs`, `auth-shape.mjs`, `input-safety.mjs`).
 * De exacte exportnamen van M3 zijn op het moment van schrijven van dit
 * prompt nog niet vastgelegd — gebruik de daadwerkelijke namen zoals M3 die
 * oplevert; bouw M3 eerst als het nog niet bestaat.
 *
 * @param {ReturnType<typeof createFakeFastify>} fastify
 * @param {Record<string, Function>} restGamesModule - M3's exports
...
#### M7d — client-/server-eventscenario (tegen M4/M5)

```js
/**
 * Toetst: onbekend event-type → `UNSUPPORTED_EVENT` (via M4's
 * `resolveEventValidator`), de snapshot-invariant ("geen correct antwoord
 * van een actieve ronde", M5d), en de `round:progress`-throttle (M5e,
 * `throttleRoundProgress`, max 2x/seconde). Bouw M4/M5 eerst als de
...
#### M7e — reconnect-scenario + pauze-op-recovery (tegen M6)

```js
/**
 * Toetst de backoff-reeks, de niet-herverzenden-regel en de
 * socketauth-hergebruikwrapper uit M6 (`server/protocol/reconnect.mjs`),
...
| --- | --- | --- | --- |
| 1 | M7a | `createFakeFastify().route(...)` + `.inject(...)` voor een simpele GET | `statusCode`/`payload` komen ongewijzigd terug, geen netwerkcode aangesproken |
| 2 | M7a | `createFakeSocketServer().connect(authPayload)` + `onConnection`-handler | server ontvangt exact dezelfde `authPayload` als de client verstuurde |
| 3 | M7a | `toRoom(id).emit(...)` met twee sockets in room A en één in room B | alleen de twee A-sockets ontvangen het event |
| 4 | M7a | `createInMemoryActionStore()` tegen M1's echte `resolveDuplicateAction` | werkt zonder aanpassing — bewijst dat de fake store het `ActionStore`-contract correct implementeert |
| 5 | M7a | `socketServer.restart()` | alle bestaande verbindingen/rooms zijn weg; een nieuwe `connect()` na restart werkt weer normaal |
| 6 | M7b | create → join → `round:answer` (actionId A) → ack | `ok: true`, `mutationCount === 1` |
| 7 | M7b | retry met dezelfde actionId A | identieke ack als in rij 6, `mutationCount` blijft `1` |
| 8 | M7b | nieuwe actionId B, zelfde antwoordinhoud, ná acceptatie | `ALREADY_ANSWERED`, `mutationCount` blijft `1` |
| 9 | M7b | nieuwe actionId C, ánder antwoord, ná acceptatie | `ALREADY_ANSWERED`, `mutationCount` blijft `1` |
| 10 | M7b | payload groter dan de afgesproken limiet, vóór envelope-parse | geweigerd door `assertPayloadSize`, `parseClientEnvelope` wordt niet aangeroepen |
| 11 | M7c | `POST /api/v1/games` met een geldig voorbeeldpayload uit `PROTOCOL.md` | 2xx, responsvorm exact zoals het `PROTOCOL.md`-voorbeeld |
| 12 | M7c | `POST /api/v1/games/join` met zowel `inviteId` als `gameCode` tegelijk | validatiefout — "precies één locator" wordt geschonden |
| 13 | M7c | `GET /api/v1/games/{code}/state` zonder `Authorization`-header | `auth-shape`-afwijzing (bijv. `TOKEN_INVALID`), geen 5xx |
| 14 | M7c | `GET /api/v1/time` | responsvorm `{ serverTime: number }` |
| 15 | M7c | `POST /api/v1/games/{code}/leave` met een hostsessie zonder spelerrol | `NOT_PLAYER`-achtige afwijzing |
| 16 | M7d | onbekend event-type naar de socketserver | `UNSUPPORTED_EVENT` via `resolveEventValidator` |
| 17 | M7d | snapshot direct na join tijdens `ROUND_ACTIVE` | bevat geen correct antwoord van de actieve ronde (snapshot-invariant) |
| 18 | M7d | 5 aanroepen van de `round:progress`-broadcastpoging binnen 1 seconde, zelfde ronde | `throttleRoundProgress` staat maximaal 2 emissies toe |
| 19 | M7d | `round:answer`-payload met een `sessionToken`-achtig veld erin | geweigerd (cross-cutting negatieve test uit M4, Basisregel 3) |
| 20 | M7e | socket valt weg, client doet 6 reconnectpogingen | vertragingen exact `[1, 2, 4, 8, 16, 30]` volgens M6's `backoffDelaySeconds` |
| 21 | M7e | reconnect terwijl het laatste antwoord al een ack had | niet opnieuw verzonden (`resolveReconnectResend` → `resend: false`) |
| 22 | M7e | reconnect zonder ontvangen ack op het laatste antwoord | exact dezelfde `actionId` herhaald, nooit een nieuwe |
| 23 | M7e | `socketServer.restart()` midden in `ROUND_ACTIVE` | room-fase wordt `PAUSED`; na reconnect + snapshot toont de client `PAUSED` gevolgd door hervatten met een nieuwe, kortere countdown — geen fase wordt stilzwijgend overgeslagen |
| 24 | M7e | reconnect-handshake ná een restart | `buildReconnectSocketAuth` gebruikt exact hetzelfde `{sessionToken, protocolVersion}`-schema als de eerste handshake, geen apart schema |

### Niet in scope voor M7
...
- Herimplementeren van M1–M6's eigen logica binnen dit harnas — de scenario's roepen
...
- Een echte Redis-herstelimplementatie bouwen: `architecture-plan` kent hiervoor nog
  ...
  proza, als brontekst) — M7e bouwt daarom voorlopig een eigen fake-stand-in in
...
- Elke sub-batch (M7a–M7e) is een eigen actie/commit binnen de autonomiegrens uit
  `CLAUDE.md` (≤5 bestanden, ≤400 regels); geen enkele batch overschrijdt dat budget.
- Waar een afhankelijke module (M2–M6) nog niet bestond bij aanvang van een batch, is
  die eerst gebouwd volgens zijn eigen faseprompt — niet lokaal binnen dit harnas
  nagebootst met een permanent karakter (tijdelijke fakes voor M7a zelf, zoals de
