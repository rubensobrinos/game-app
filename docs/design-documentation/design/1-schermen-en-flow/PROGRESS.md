...
`S09` en `S10` (Echt of Nep, Hoger of Lager) staan nu op niveau 1 (**gebouwd,
prompt 14**, 3 aug 2026). Ze stonden hier eerder als "buiten de lanceerscope"
— dat was fout (principe 8): `PRODUCT.md`'s §Spelvormen in multiplayer noemt
`real_or_fake_flag` (S09) en `higher_lower` (S10) expliciet als twee van de
vijf **Golf 1 — MVP-launch**-spelvormen, naast `flags_mc` (S08). De server- en
protocolkant bleken al volledig te bestaan en getest te zijn:
`server/rules/question-selection.js`, `server/rules/validators.js`
(`{choice:'real'|'fake'}` resp. `{side:0|1}`) en `PROTOCOL.md`'s
`round:started`-voorbeelden per `gameType`. Los van deze twee schermen blijft
`client/flow/`'s spelvorm-selector vandaag vastzitten op `['flags_mc']`
(`UI-17`, `HANDOFF-UI.md`; al gesignaleerd naar `product-plan`'s eigenaar in
`docs/game-flow-plan/GF-HANDOFF-TO-INT-A.md`) — dat blokkeerde dit prompt niet
(zie prompt 14 voor hoe getest is zonder die selector). Zie
[`14-S09-S10-echt-of-nep-en-hoger-of-lager.md`](prompts/14-S09-S10-echt-of-nep-en-hoger-of-lager.md).
...
| S04 | Naam kiezen | 1 | **Alle eerder gevonden gaten nu gedicht (3 aug 2026, kleine correctie na prompt 06):** sociaal bewijs bleek al gebouwd (thema 4); tekenteller (prompt 06); nu ook: (1) **roombevestiging** ("Je doet mee aan game 482 917", `04`'s inhoudspunt) — alleen zichtbaar ná code-invoer (`state.locator.type === 'code'`, `formatCode()` uit `room-header.mjs` hergebruikt), want `GET /api/v1/games/preview` (de invite-linkroute) levert alleen `roomId` terug, geen `gameCode` (`PROTOCOL.md`) — een échte protocolgrens, geen keuze hier, dus voor die route toont dit scherm 'm bewust niet; (2) **teller kapt nu écht live af** — `join-state.mjs`'s `sanitizeDisplayName` kapte al stil af bij `SUBMIT`, maar het zichtbare veld zelf bleef ongewijzigd tot 60 tekens (`nameInput.maxLength`) en de teller telde die rauwe invoer (kon "25/20" tonen). `join.mjs` synchroniseert het veld nu terug naar `state.displayName` zodra die verschilt van wat getypt is — geen nieuwe validatielaag, alleen de al-bestaande afkap ook zichtbaar maken. Geverifieerd met een losse testharnas: 25 tekens typen geeft een veld + teller die op 20 stoppen, een normale korte naam blijft ongestoord getypt. |
...
| S09 | Echt of Nep | 1 | **Gebouwd (prompt 14):** `gameplay.mjs` takt nu af op `gameType`, `round-model.mjs` kent `selectChoice`/`answerPayloadFor` (`{choice:'real'|'fake'}`). `kind: "real"` hergebruikt de bestaande vlag-`<img>`; `kind: "generated"` tekent via het nieuwe `flag-renderer.mjs` (poort van de singleplayer-app's canvasrenderer, dezelfde `flag-renderer-1`-vocabulaire als `shared/content/flag-spec.mjs`). Geverifieerd via een losse testharnas (geen `client/flow/`-selector nodig, zie prompt 14): beide vraagvarianten renderen, `Echt`/`Nep`-knoppen sturen de juiste waarde, correcte/foute knop krijgt de juiste markering. Niet naar niveau 2: geen eigen compositie-pas, alleen de bestaande shell hergebruikt. |
| S10 | Hoger of Lager | 1 | **Gebouwd (prompt 14):** twee zijden van het duel zijn zelf de knop (`04`'s "directe side-select"), elk met eigen vlag + landnaam; `round-model.mjs`'s `selectSide`/`answerPayloadFor` sturen `{side:0|1}`. **Kanttekening gevonden tijdens het bouwen:** `PROTOCOL.md` suggereert dat de rauwe metriekwaarden in `round:ended` meegaan (`resultDetails.values`) — geverifieerd tegen de daadwerkelijke `server/transport/socket.mjs`'s `runEndRound()`, en dat veld wordt vandaag niet meegestuurd naar de client. De reveal toont daarom alleen wélke zijde/land correct was (uit `correctAnswer.side`, wat wél verstuurd wordt), geen absolute cijfers — een eerlijke tekst-equivalent, niet de rijkere reveal die `PROTOCOL.md`'s tekst impliceert. Dat is een documentatie/implementatie-mismatch voor `PROTOCOL.md`'s eigenaar, geen thema-1-punt. Sociale headline (S14) vuurt bewust niet voor deze spelvorm: `social-headline.mjs` verwacht een string-`correctOptionId`, een numerieke `side` valt daar stil op terug naar `null` — geen crash, wel een bekende beperking, niet opgelost hier (aparte generalisatie van een gedeeld bestand, buiten deze prompt's scope). |
...
| # | Scherm | Niveau | Wat er nog mist |
|---|---|---|---|
| S16 | Pauze | 1 | **Bereikbaarheidsbug opgelost (prompt 01):** de hostbalk (vergrendelen/verwijderen/beëindigen/hervatten) verplaatst zichzelf nu ín de pauze-overlay zolang `PAUSED` actief is (`session-shell.mjs`'s `renderPauseOverlay`/`restoreHostBarPosition`, geverifieerd met Playwright: alle drie de knoppen bereikbaar, vergrendelen tijdens pauze houdt de overlay open, hervatten zet de balk terug in de normale flow) — geen losse duplicaatknoppen meer nodig, dat probleem was eerder alleen voor Hervat opgelost. **Tweede bereikbaarheidsbug gevonden en opgelost (buiten een prompt, 3 aug 2026):** `02-S05-permanente-qr-code.md` was al uitgevoerd (de code/QR zitten sindsdien permanent in `#app-header`), maar `.session-pause-overlay` (`z-index: 70`) lag ver boven `#app-header` (`z-index: 60`) — de overlay dekte de header dus onbedoeld af zodra `PAUSED` inging, en maakte `qrButton` fysiek onklikbaar (niet alleen visueel verborgen: `document.elementFromPoint()` op de knop loste op naar de overlay, niet de knop). Dat is een directe schending van `D-018` ("blijft het hele potje zichtbaar"), dus bugfix, geen ontwerpvraag. Verlaagd naar `z-index: 55` (`base.css`) — blijft boven de gewone content, onder `#app-header` en onder `.room-qr-overlay` (75). Geverifieerd tegen de echte server (niet de mock): host alleen een match starten, countdown laten aflopen, pauzeren, QR-knop daadwerkelijk aanklikken (Playwright's `click()` faalt bij occlusie) — modal opent. Blijft op niveau 1: dit was alleen het al-bestaande criterium weer bruikbaar maken, geen nieuwe compositie. |
...
| S18 | Voorkeuren | 1 | Taal en thema werken en zijn toegankelijk. Zwevend paneel in plaats van bottom sheet — expliciet afgeraden in het benchmarkrapport §9. **Niet zomaar door thema 1 te bouwen (gecontroleerd 3 aug 2026):** `T2-9-overlays-bottom-sheets.md` (thema 2) dekt exact dit component en zit zelf ⏸ vast op `UI-20` (`HANDOFF-UI.md`, nog open — thema 2 en thema 5 claimen hetzelfde onderdeel). Wachten op die beslecht, niet zelf een tweede bottom-sheet-poging naast T2-9 bouwen. Geen geluidsinstelling — dit stuk (los van de sheet-vorm) is wel vrij bouwbaar, zie `preferences.mjs`'s bestaande `loadMuted`/`saveMuted`. |
