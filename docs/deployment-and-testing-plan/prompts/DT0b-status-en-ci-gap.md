# Prompt — DT0b: Statusregel + canoniek testcommando + CI-kloof

Onderdeel van [`docs/deployment-and-testing-plan/README.md`](../README.md), fase
DT0b. Doel: één gerichte aanvulling in dat README nadat DT0 is afgerond — bewust los
van DT0's eigen bestandenbudget (zie [`REVIEW.md`](REVIEW.md) #1).

## Context

- Vereist: DT0 is afgerond, d.w.z. `tests/{contract,integration,e2e,load,chaos}/`
...
- [`REVIEW.md`](REVIEW.md) #9: de bestaande, devkit-managed CI
  ...
  stilzwijgend blijven staan terwijl DT1–DT3 lokaal groen worden.

## Stappen

1. Voeg in `docs/deployment-and-testing-plan/README.md` een korte statusregel toe
   (bijvoorbeeld onder de DT0-alinea in Fasering) die vermeldt: DT0 is afgerond, de
...
3. Herhaal expliciet de CI-kloof: de managed CI dekt deze boom niet; dat wordt pas
   opgelost na een goedgekeurd DT7-voorstel. Tot die tijd draaien deze tests
...
- README bevat een statusregel dat DT0 is afgerond, met locatie en
  verificatiecommando.
- README documenteert het canonieke testcommando-sjabloon per laag én de expliciete
  CI-kloof, met een concrete verwijzing naar DT7 als plek waar dat wordt opgelost.
