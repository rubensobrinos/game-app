...
| Testlagen — Loadtests | 📄 klaar / ✅ k6-target compleet, uitvoering blijft `prod` | DT5 Deel 1: 10/10 criteria toegewezen aan een bewijsmethode (📄). **DT-R4-herverificatie (2026-08-02, nacht, tweede ronde): k6-target bestaat nu volledig** — de socketlaag hangt aantoonbaar aan een luisterende server (attach bevestigd) én de keten-test over echt HTTP/WebSocket-verkeer (`full-match-transport.test.mjs`) draait 6/6 groen (was 2/4 rood door een `startMatch`-regressie die inmiddels elders is opgelost). Nog 0/10 criteria daadwerkelijk gemeten: DT5 §Deel 2 (scripts schrijven in `tests/load/`) kan starten onder het bestaande `deps`-akkoord; §Deel 3 (uitvoeren) blijft `prod` en vereist een eigen, apart akkoord |
