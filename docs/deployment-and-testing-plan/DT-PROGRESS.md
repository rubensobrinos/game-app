...
| Testlagen — Restart-/chaostests | 📄 klaar / 🚧 scenario 1 3× geprobeerd, nu geblokkeerd op een INT-18-bug | DT6: `aseso-game-chaos`-stack draait geïsoleerd (`compose.chaos.override.yml`, loopback 8080/8443) naast de echte `aseso-game`-stack. **Run 1:** geen socketlaag, alleen REST-deel; roomstate overleeft niet (geen Redis-koppeling). **Run 2 (apart akkoord):** wél "midden in een ronde", nog geen Redis-koppeling in `server/index.mjs` — roomstate overleeft niet, oorzaak nu eenduidig bevestigd (`redis-cli dbsize` bleef `0`). **Run 3 (2026-08-02, na het landen van de storekeuze op `REDIS_URL`):** stack herbouwd, `/readyz` bevestigt `{"ok":true,"store":"redis"}` — maar de eerste `POST /api/v1/games` faalt al met `500`. Oorzaak: de versioned tokenhash (`v1:<hex>`) bevat `:`, wat `redis-keys.js`'s segmentvalidator verbiedt — de Redis-store is hierdoor voor 100% van het verkeer non-functioneel, los van chaos. Gemeld als **INT-18** in `docs/integration-plan/HANDOFF.md` (INT-B's eigen DoD-test voor de storekoppeling). Niet zelf gefixt. Stack blijft draaiend voor directe herhaling zodra INT-18 is opgelost. Scenario 2–6: elk apart geautoriseerd. |
